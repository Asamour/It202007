...
if(isset($_POST["save"])){
        ...
        $db = getDB();
        if(isset($id)){
                $stmt = $db->prepare("UPDATE Survey set title=:title, description=:description, visibility=:visibility, modified=:modified where id=:id");
...
<form method="POST">
        ...
        <select name="visibility" value="<?php echo $result["visilibty"];?>">
